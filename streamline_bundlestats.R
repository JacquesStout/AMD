...
if (Sys.info()[['user']]=='jas') {
  ...
  if (project == 'AMD') {
    mainpath = '/Volumes/dusom_mousebrains/All_Staff/Analysis/AMD_farunfull/'
    outputpath= '/Users/jas/jacques/Whiston_article/R_figures_May23_farunfull/'
    outputpath= '/Users/jas/jacques/Whiston_article/AMD_submission_frontiers/Edited_May/Figure_5/predata/'
  }
}
...
if (mrtrix == 1){
  param_str = '_mrtrix'
  folderName <- basename(outputpath)
  newFolderName <- paste0(folderName, "_mrtrix/")
  outputpath <- file.path(dirname(outputpath), newFolderName)
  regions_all <- vector(mode = 'list', length = 6)
  regions_all <- list(list(62,28),list(28, 9), list(62, 1))
  regions_num = 3
  ...
  regions_all <- list(list(62,28),list(58, 45), list(28, 9), list(62, 1))
  regions_num = 4
}
...
csvpath_folder = path_join(list(mainpath, paste('Statistics',stat_type,'_distance_',distance_val,space_param,'_non_inclusive_symmetric',as.character(folder_str),param_str,sep='')))

myYear = 'Initial'
if (myYear=='Initial'){
  #groups =list('Paired Initial AMD','Paired Initial Control')
  groups =list('Initial_AMD','Initial_Control')
}
if (myYear=='2Year'){
  #groups =list('Paired 2-YR AMD','Paired 2-YR Control')
  groups =list('2Year_AMD','2Year_Control')
}
#groups =list('Paired 2-YR AMD','Paired 2-YR Control')
#groups =list('Paired Initial AMD','Paired Initial Control')
...
for (z in 1:regions_num) {
  ...
  df3<-data.frame(g3)
  names(df3)[names(df3) == "genotype"] <- "group"
  df3<-cbind(df3,group=groups[[1]])
  
  df4<-data.frame(g4)
  names(df4)[names(df4) == "genotype"] <- "group"
  df4<-cbind(df4,group=groups[[2]])
  ...
  for (i in 1:length(clus3bundleid)){
    dftemp3<-subset(df, (group==groups[[1]] & Bundle.ID==clus3bundleid[i]))
    dftemp4<-subset(df, (group==groups[[2]] & Bundle.ID==clus4bundleid[i]))
    ...
    dfstream3Bundlei<-subset(df, (group==groups[[1]] & Bundle.ID==clus3bundleid[i]))
    dfstream4Bundlei<-subset(df, (group==groups[[2]] & Bundle.ID==clus4bundleid[i]))
    dfstreamBundlei<-rbind(dfstream3Bundlei,dfstream4Bundlei)
    dfstreamBundlei$genotypenorm <- ((-1)*as.numeric(dfstreamBundlei$group) - 3.5)
    ...
    mytLength<-t.test(Length ~ group, data = dfstreamBundlei)
    mytCoherence<-t.test(Streamline.coherence ~ group, data = dfstreamBundlei)
    mytFA<-t.test(fa ~ group, data = dfstreamBundlei)
    if (onlyfa==0){
    mytMD<-t.test(md ~ group, data = dfstreamBundlei)
    mytAD<-t.test(ad ~ group, data = dfstreamBundlei)
    mytRD<-t.test(rd ~ group, data = dfstreamBundlei)
    }
    ...
    ggplot(dfstreamBundlei, aes(y=fa, x=Point.ID, color=group)) +
      geom_smooth(data=dfstreamBundlei, se = TRUE, level=0.95, aes(color=group,fill=group))+
    ...
    m03 <- lm(fa~poly(Point.ID,4)*group, data=dfstreamBundlei)
    ...
    ggplot(dfstreamBundlei, aes(y=m03$fitted.values, x=Point.ID, color=group)) +
      geom_smooth(aes(ymin=LoCI,ymax=HiCI,fill=group, colour=group),stat = 'identity')+
      ggtitle(paste('FA along streamlines', toString(i), sep= ' '))+
      theme_classic()+
      scale_color_manual(values=c(group_colors[1],group_colors[2]))+
      scale_fill_manual(values=c(group_colors[1],group_colors[2]))+
